...
5. [4) Claude Code manual step (skip if setup script already ran)](#4-claude-code-manual-step-skip-if-setup-script-already-ran)
...
# Load Claude->Ollama environment generated by the setup script
source scripts/claude_ollama_env.sh

# Create/refresh local aliases used by Claude + verify installed models
