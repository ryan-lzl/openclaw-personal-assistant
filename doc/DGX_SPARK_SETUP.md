...
0. [0) Assumptions](#0-assumptions)
1. [1) Quick path (recommended): run repo setup scripts](#1-quick-path-recommended-run-repo-setup-scripts)
2. [2) Start Nemotron3-Nano model server for PM agent (optional; choose one, mutually exclusive)](#2-start-nemotron3-nano-model-server-for-pm-agent-optional-choose-one-mutually-exclusive)
...
# (A) Start Ollama background service + pull/alias qwen3-coder models + generate Claude env helper
./scripts/setup_ollama_claude.sh

# Load Claude->Ollama environment into your current shell (only needed once per shell session)
source scripts/claude_ollama_env.sh

# If you want Nemotron PM locally (choose ONE):
...
- `scripts/setup_ollama_claude.sh`
  - Ensures Node.js >= 22 and npm are available (for `ollama launch openclaw`)
  - Ensures npm global prefix is user-writable (avoids `EACCES` during OpenClaw install)
  ...
  - Pulls `qwen3-coder:30b` and `qwen3-coder-next:latest`
  - Creates/refreshes aliases `qwen3-coder` and `qwen3-coder-next`
  - Prints `ollama list` to verify installed aliases/models
  - Warms up `qwen3-coder` automatically (equivalent to your manual `claude --model qwen3-coder` warmup step)
  - Bootstraps OpenClaw via `ollama launch openclaw --config` only when OpenClaw config is missing
  - If `http://127.0.0.1:8000/v1/models` is reachable, auto-configures OpenClaw PM provider to that vLLM-compatible endpoint
  - Syncs `agent:main:main` session model to the configured PM default and restarts gateway when needed
  - Starts/restarts OpenClaw gateway service for normal use (`openclaw gateway start/restart`)
  - Auto-launches `openclaw tui` in interactive shells at the end of setup
...
## 4) Claude Code manual step (skip if setup script already ran)

If you already ran `./scripts/setup_ollama_claude.sh` in Section 1, skip this section and continue to Section 5.
That script already configures Ollama background service and generates `scripts/claude_ollama_env.sh`.
...
- `scripts/run_claude_task.sh <packet>`
- (Do not allowlist `bash scripts/run_claude_task.sh <packet>`)

Everything else should be denied by policy.

Recommended strict setup (direct script call only; no `bash` prefix):

```bash
# Enable exec tool while keeping messaging profile.
openclaw config set tools.alsoAllow '["exec"]' --strict-json
openclaw config set tools.exec.host gateway
openclaw config set tools.exec.security allowlist
openclaw config set tools.exec.ask always

# Replace approvals with strict allowlist + always-ask policy.
cat <<'JSON' | openclaw approvals set --stdin
{
  "version": 1,
  "socket": {},
  "defaults": {
  ...
  "agents": {
    "main": {
      ...
      "allowlist": [
        {
          "pattern": "/home/ryan/workspace/openclaw-personal-assistant/scripts/run_claude_task.sh"
        }
      ]
    }
  }
...
With this policy, `scripts/run_claude_task.sh <packet>` is allowed (with approval prompt),
while `bash scripts/run_claude_task.sh <packet>` is denied as an allowlist miss.
...
## 9) PM ↔ Coding Agent Flow (Mermaid)
